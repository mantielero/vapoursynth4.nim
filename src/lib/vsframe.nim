...
import constants, ../wrapper/vapoursynth_wrapper
...
proc toFormat*(format:VSVideoFormat):Format =
  # Get name
  #if format == nil:
...
proc getColorFamily*(node:ptr VSNode):VSColorFamily =
  let vinfo       = API.getVideoInfo(node)
  vinfo.format.colorFamily.VSColorFamily


proc getVideoInfo*(node:ptr VSNode):VideoInfo =
    let vinfo = API.getVideoInfo(node)
...
proc getVideoFrameFormat*(frame:ptr VSFrame):ptr VSVideoFormat =
  doAssert( frame != nil, "called with a nil pointer")
  #if frame == nil:
  #  raise newException(ValueError,"called with a nil pointer")
  result = API.getVideoFrameFormat(frame) 

proc width*(frame:ptr VSFrame, plane:int):cint =
...
proc height*(frame:ptr VSFrame, plane:int):cint =
...
proc getFrame*(node:ptr VSNode, frame_number:int):ptr VSFrame =
...
proc freeFrame*(frame:ptr VSFrame) =
...
proc getReadPtr*(frame:ptr VSFrame, plane:int):pointer =
  return API.getReadPtr(frame, plane.cint)

proc getWritePtr*(frame:ptr VSFrame, plane:int):pointer =
  return API.getWritePtr(frame, plane.cint)

proc getStride*( frame: ptr VSFrame, plane:int ):uint =
...
#[
proc getPlane*(frame:ptr VSFrame, plane:Natural):Plane =
...
#[
proc `[]`*(frame:ptr VSFrame, plane:Natural):Plane =
  getPlane(frame, plane)

iterator planes*(frame:ptr VSFrame):Plane =
...
proc newVideoFrame*(src:ptr VSFrame,width:Natural,height:Natural):ptr VSFrame =
  let fi = API.getVideoFrameFormat(src)
...
#[
proc `[]`*(frame:ptr VSFrame, plane:Natural):proc(row:int):ptr uint8 =
...
proc numPlanes*(frame:ptr VSFrame):cint =  # Evitamos conversiones innecesarias
  ## Number of planes in a frame
  API.getVideoFrameFormat( frame ).numPlanes  # Frame information


proc getPtr(frame:ptr VSFrame, plane:cint):uint =
...
proc stride(frame:ptr VSFrame, plane:cint):uint =
  API.getStride( frame, plane ).uint
#type
#  Row = ptr UncheckedArray[uint8]

proc `[]`*(frame:ptr VSFrame, plane:cint, row:uint):ptr UncheckedArray[uint8] =
...
proc `[]`*(frame:ptr VSFrame, plane:cint, row:cint):ptr UncheckedArray[uint8] =
  let ini:uint = frame.getPtr(plane)            # Don't use other thing here
  let stride:uint = frame.getStride( plane )
  result = cast[ptr UncheckedArray[uint8]]( ini + row.uint * stride )  

proc `[]`*(frame:ptr VSFrame, plane:cint ):Plane =
...
proc ggetPlane*(frame:ptr VSFrame, plane:Natural):Pplane =
  # Only for uint8 (right now)
  if frame == nil:
    raise newException(ValueError, "called with nil pointer")
  let frameFormat = getVideoFrameFormat(frame)
  if plane > frameFormat.numPlanes-1:
    raise newException(ValueError, "the plane requested is above the number of planes available")
